...
const setMatchesButton = document.getElementById('setMatchesButton');
...
function checkTournamentState() {
    ...
    if (savedPlayers && playedMatches) {
        ...
        renderTournamentStatus(matches, tournamentPlayers);
    }
    else {
...
function renderTournamentStatus(playedMatches, players) {
...
function updatePlayersList() {
    ...
    if (tournamentPlayers.length >= 3) {
        countElement === null || countElement === void 0 ? void 0 : countElement.classList.add('ready');
        setMatchesButton.disabled = false;
    }
    else {
        countElement === null || countElement === void 0 ? void 0 : countElement.classList.remove('ready');
...
startTournamentButton.addEventListener('click', () => {
    ...
    if (!bracket || !bracket.rounds.length || !((_a = bracket.rounds[0]) === null || _a === void 0 ? void 0 : _a.length)) {
        return;
    }
...
function updateBracketWithWinners(bracket, playedMatches) {
    var _a;
    ...
    for (let roundIdx = 1; roundIdx < bracket.rounds.length; roundIdx++) {
        ...
        for (let matchIdx = 0; matchIdx < round.length; matchIdx++) {
            ...
            if (match.player1 && match.player1.includes('Winner of Match')) {
                const playedMatch = playedMatches.find((m) => m.roundIndex === roundIdx - 1 && m.matchIndex === prevMatch1Idx);
                if (playedMatch) {
                    match.player1 = playedMatch.winner;
                }
            }
            if (match.player2 && match.player2.includes('Winner of Match')) {
                const playedMatch = playedMatches.find((m) => m.roundIndex === roundIdx - 1 && m.matchIndex === prevMatch2Idx);
                if (playedMatch) {
                    ...
                    match.player2 = ((_a = prevRound[prevMatch2Idx]) === null || _a === void 0 ? void 0 : _a.player1) || null;
                }
            }
        }
    }
...
if (continueTournamentButton) {
    continueTournamentButton.addEventListener('click', () => {
        ...
        for (let roundIdx = 0; roundIdx < bracket.rounds.length; roundIdx++) {
            const round = bracket.rounds[roundIdx];
            for (let matchIdx = 0; matchIdx < round.length; matchIdx++) {
                const matchPlayed = playedMatches.find((m) => m.roundIndex === roundIdx && m.matchIndex === matchIdx);
                if (!matchPlayed) {
