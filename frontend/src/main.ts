...
class TranscendanceApp {
  ...
  constructor() {
    ...
    if (!container) throw new Error('#app veya #output bulunamadı');
  ...
  private async renderPage(title: string, htmlFile: string, cssFile: string) {
    ...
    try {
      ...
      if (!response.ok) throw new Error('Sayfa bulunamadı: ' + htmlFile);
...
private async executePageScripts(htmlFile: string) {
