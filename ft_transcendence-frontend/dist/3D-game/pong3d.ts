...
export class Pong3DGame {
    ...
    private getStorageData() {
        ...
        if (this.gameData.matchType === 'h2ai') {
            ...
            this.gameData.rightPlayer = sessionStorage.getItem('playerName') || 'Oyuncu';
        }
        else {
            playersInfo.otherPlayer_id = invitedUserId ? parseInt(invitedUserId, 0) : null;
            this.gameData.leftPlayer = sessionStorage.getItem('player1') || 'Oyuncu 1';
            this.gameData.rightPlayer = sessionStorage.getItem('player2') || 'Oyuncu 2';
        }
    ...
    private async init(): Promise<void> {
        try {
            ...
            this.gamePanel.update({
                ...
                gameMode: this.gameData.matchType === 'h2h' ? 'Human vs Human' : 'Human vs AI',
                winningScore: this.winningScore,
                controls: [
                    "W / S - Sol Paddle",
                    "Yukarı / Aşağı - Sağ Paddle",
    ...
    private async loadModel(): Promise<void> {
        try {
            ...
            if (modelCameras.length > 0) {
                ...
                console.warn('Modelde kamera bulunamadı, varsayılan kamera kullanılacak');
            }
            ...
            if (!this.screenMesh) {
                console.warn('Screen mesh bulunamadı. Mevcut mesh\'ler:');
                result.meshes.forEach(mesh => {
                    console.warn('Mesh adı:', mesh.name);
                });
            }
    ...
    private setupScreenTexture(): void {
        if (!this.screenMesh) {
            console.warn('Screen mesh bulunamadığı için texture oluşturulamıyor');
            return;
        }
