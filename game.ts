...
type GameMode = 'h2ai' | 'h2h' | 'tournament';
...
function showGame(mode: GameMode) {
    currentGameMode = mode;
    // Update game mode label
    const modeLabels: { [key in GameMode]: string } = {
        'h2ai': 'vs AI',
        'h2h': 'vs Human',
...
function gameLoop() {
    ...
    // Paddle 2 collision
    if (
        ballX + ballSize >= paddle2.offsetLeft &&
...
function endGame(winner: string, player1FinalScore: number, player2FinalScore: number) {
    gameRunning = false;
    startButton.textContent = 'Start Game';
