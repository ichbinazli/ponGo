...
function applyMatchResult() {
    ...
    // ── Apply completed match data from API response ──────────────────
    if (apiResponseRaw) {
        try {
            ...
            // ── If round completed, merge next round matches from backend ──
            if (apiResponse.data?.roundCompleted && apiResponse.data?.nextRound?.matches) {
                ...
                // Remove any placeholder matches for this next round
                matches = matches.filter(m => m.round !== nextRound);

                // Add the backend-provided next round matches
        ...
        // Fallback: apply from sessionStorage values if API response missing
...
function getMatchStatus(match: Match): { text: string; className: string } {
    if (match.winner || match.winner_participant_id) {
        return { text: 'Tamamlandı', className: 'bg-emerald-900 text-emerald-300' };
    }

    if (match.player1?.alias && !match.player2?.alias) {
        if (match.round === 1) {
            return { text: 'BYE - 2. tura geçti', className: 'bg-amber-900 text-amber-300' };
        }
        return { text: 'Rakip bekleniyor', className: 'bg-slate-700 text-slate-200' };
    }

    if (match.player1?.alias && match.player2?.alias) {
        return { text: 'Hazır', className: 'bg-cyan-900 text-cyan-300' };
    }

    return { text: 'Bekliyor', className: 'bg-slate-700 text-slate-200' };
}
...
function selectMatchForStart(matchIndex: number) {
    ...
    setTournamentStatus({
        title: 'Maç seçildi',
        detail: `${matches[matchIndex].player1.alias} vs ${matches[matchIndex].player2?.alias} maçı başlatılmayı bekliyor.`
    });
}
...
function bindStartSelectedMatch() {
    ...
    startMatchButton.onclick = () => {
        ...
        // Save tournament state so it can be restored after the match
        saveTournamentState();
...
function renderTournamentBracket() {
    ...
    if (!matches.length) {
        ...
        emptyState.textContent = 'Henüz maç oluşturulmadı.';
    ...
    roundNumbers.forEach((round) => {
        ...
        roundBadge.textContent = `${roundMatches.length} maç`;
        ...
        roundMatches.forEach((match, index) => {
            ...
            matchTitle.textContent = `Maç ${match.matchOrder ?? index + 1}`;
            ...
            if (ready) {
                ...
                selectButton.textContent = isSelected ? 'Seçildi' : 'Maçı Seç';
...
export async function initTournament() {
    ...
    // ── Returning from a tournament match ────────────────────────────
    ...
    if (isReturningFromMatch && hasStoredState) {
        ...
        if (readyMatchCount > 0) {
            setTournamentStatus({
                ...
                detail: `${readyMatchCount} hazır maç var. Sonraki maçı seçip başlatabilirsiniz.`
            });
        } else {
            ...
            if (allDone && finalMatch?.winner) {
                setTournamentStatus({
                    ...
                    detail: `Kazanan: ${finalMatch.winner.alias}`
                });
                sessionStorage.removeItem('tournamentState');
            } else {
                setTournamentStatus({
                    title: 'Bekleniyor',
                    detail: 'Hazır maç bulunmuyor. Önceki turların tamamlanması bekleniyor.'
                });
            }
        }
        return;
    ...
    setTournamentStatus({
        title: 'Başlatılıyor',
        detail: 'Turnuva oluşturuluyor ve oyuncular ekleniyor...'
    });
...
async function initAsync() {
    setTournamentStatus({
        title: 'Turnuva oluşturuluyor',
        detail: 'Turnuva kaydı API üzerinde oluşturuluyor...'
    });
    ...
    setTournamentStatus({
        ...
        detail: 'Katılımcılar turnuvaya kaydediliyor...'
    });
    ...
    setTournamentStatus({
        ...
        detail: 'İlk tur ve devam turları oluşturuluyor...'
    });
...
async function createFirstRound() {
    ...
    // console.log("Starting tournament with payload: ", payload);
    // console.log("Matches to start: ", tournamentMathches);
    ...
    setTournamentStatus({
        ...
        detail: readyMatchCount > 1
            ? `${readyMatchCount} hazır maç var. İstediğiniz maçı seçip başlatabilirsiniz.`
            : readyMatchCount === 1
                ? '1 hazır maç var. Maçı başlatabilirsiniz.'
                : 'Hazır maç bulunmuyor.'
    });
    console.log("Matches after API response: ", matches);
}
